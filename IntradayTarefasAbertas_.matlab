...
#"Tipo Alterado4" = Table.TransformColumnTypes(#"Dividir Coluna pelo Delimitador",{{"GRA.1", type text}, {"GRA.2", type text}, {"GRA.3", type text}, {"GRA.4", Int64.Type}}),
    ...
    #"Consultas mescladas1" = Table.NestedJoin(#"Personalização Adicionada5",{"MATRICULATECNICO"},TecnicosClick,{"MATRÍCULA"},"TC"),
    #"TC Expandido" = Table.ExpandTableColumn(#"Consultas mescladas1", "TC", {"PERFILTECNICO"}, {"TC.PERFILTECNICO"}),
    #"Personalização Adicionada6" = Table.AddColumn(#"TC Expandido", "DifPROMESSA", each [DATAPROMESSA]-DateTime.LocalNow()),
    ...
    #"Consultas mescladas2" = Table.NestedJoin(#"Personalização Adicionada8",{"RECLAMACAO"},TBL_ApoioTup,{"DESCRIÇÃO RECLAMAÇÃO"},"NewColumn"),
    #"NewColumn Expandido1" = Table.ExpandTableColumn(#"Consultas mescladas2", "NewColumn", {"IMPACTO NO INDICADOR"}, {"IMPACTO NO INDICADOR"}),
    #"Consultas mescladas3" = Table.NestedJoin(#"NewColumn Expandido1",{"""NRBA"""},ordemAbertas,{"QT.""NRBA"""},"OR"),
    #"OR Expandido" = Table.ExpandTableColumn(#"Consultas mescladas3", "OR", {"Ordem"}, {"OR.Ordem"}),
    #"Personalização Adicionada9" = Table.AddColumn(#"OR Expandido", "Prioridadeop2", each if Text.Contains([#"Prioridade op."], "Priorit") or Text.Contains([#"Prioridade op."], "VIP") or Text.Contains([#"Prioridade op."], "Ouvidoria") then "Cliente Prioritário" else if Text.Contains([#"Prioridade op."], "Anatel") or Text.Contains([#"Prioridade op."], "JEC") or Text.Contains([#"Prioridade op."], "Procon") then "Orgãos Oficiais" else "Sem prioridade"),
    #"Duplicatas Removidas" = Table.Distinct(#"Personalização Adicionada9", {"""NRBA"""}),
    #"Consultas Mescladas" = Table.NestedJoin(#"Duplicatas Removidas",{"SETOR"},RNE_hierarquia,{"SETOR"},"NewColumn",JoinKind.LeftOuter),
    #"NewColumn Expandido" = Table.ExpandTableColumn(#"Consultas Mescladas", "NewColumn", {"COORDENADOR", "GERENTE", "GG", "GAA", "REGIONAL"}, {"COORDENADOR", "GERENTE", "GG", "GAA", "REGIONAL"}),
    #"Colunas Reordenadas" = Table.ReorderColumns(#"NewColumn Expandido",{"""NRBA""", "ESTADO", "INICIOAGENDAMENTO", "FIMAGENDAMENTO", "AREATRABALHO", "SERVICO", "ATIVIDADE", "BLOQUEADO", "CAIXA", "CONTATO1", "CONTATO2", "CONTATO3", "CONTATOEFETIVO", "DATAABERTURAOS", "DATAABERTURABA", "DATAPROMESSA", "DATAINDICADOR", "INICIOEXECUCAO", "FIMEXECUCAO", "DURACAO", "ESTACAO", "FILAPOSTO", "LATITUDE", "LONGITUDE", "NOMECLIENTE", "CIRCUITO", "DOCUMENTOASSOCIADO", "PRIORIDADE", "Prioridade op.", "QTDDPONTOS", "RECLAMACAO", "REINCIDENCIA", "REPAROGARANTIA", "SKILLS", "TECNICO", "TERMINAL", "SETOR", "UF", "PRONTOPARAEXECUCAO", "FLAGAGENDAMENTO", "MATRICULATECNICO", "TIPOCOORDENADAS", "CODIGOROTEAMENTO", "MERCADO", "VELOCIDADECONTRATADA", "CONTRATO", "HojeRef", "AtribComPeso", "Faixa", "IniAtrib", "HojeRefe", "DDifAtrib", "DifAtrib", "GRA", "GG","GERENTE","COORDENADOR", "B2", "TC.PERFILTECNICO", "DifPROMESSA", "GAA", "CN", "Hour", "Turno", "IMPACTO NO INDICADOR", "OR.Ordem", "Prioridadeop2"}),
    #"Personalização Adicionada7" = Table.AddColumn(#"Colunas Reordenadas", "DifPROMESSA.1", each [DATAABERTURAOS]+0.33333333-DateTime.LocalNow()),
    ...
    #"Consultas mescladas" = Table.NestedJoin(#"Personalização Adicionada10",{"TERMINAL"},TUP,{"TERMINAL"},"NewColumn"),
    #"NewColumn Expandido2" = Table.ExpandTableColumn(#"Consultas mescladas", "NewColumn", {"Marcado"}, {"Marcado"}),
    #"Valor Substituído2" = Table.ReplaceValue(#"NewColumn Expandido2",null,"Não Marcado",Replacer.ReplaceValue,{"Marcado"}),
    #"Consultas mescladas4" = Table.NestedJoin(#"Valor Substituído2",{"SKILLS"},perfilativdade,{"SKILLS"},"NewColumn"),
    #"NewColumn Expandido3" = Table.ExpandTableColumn(#"Consultas mescladas4", "NewColumn", {"Perfil de Atividade"}, {"Perfil de Atividade"})
in
    #"NewColumn Expandido3"